...
/*   Updated: 2024/12/23 03:52:30 by ikarouat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
const char	*check_format_specifier(const char *s, va_list *ap_p)
{
	...
	if (frmt_spec == 'd' || frmt_spec == 'i')
		...
		buffer = ft_putnbr_base(va_arg(*ap_p, (int)), "0123456789abcdef");
	else if (frmt_spec == 'p')
...
int	handle_arg(const char *s, va_list *ap_p)
{
	...
	flags = get_flags(s, &frmt_ctx);
	frmt_ctx.buffer = check_format_specifier(s, ap_p);
	frmt_ctx.buffer = apply_flags(frmt_ctx);//To Implement
